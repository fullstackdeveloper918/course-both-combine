...
const BadgesTable = ({ badges, setBadges, totalPages, currentPage, onPageChange }) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(result.message || "Failed to delete badge.", { autoClose: 2000 });
      }
    } catch (error) {
      ...
      {badges?.length > 0 ? (
        <>
          <table className="w-full border-collapse bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
            ...
            <tbody>
              {badges?.map((badge) => (
                <tr key={badge.badge_id} className="border-b dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-300">
                  <td className="p-4 text-gray-900 dark:text-white">{badge.badge_name}</td>
                  <td className="p-4 text-gray-900 dark:text-white">{badge.points_required}</td>
                  <td className="p-4 flex gap-2">
                    <button onClick={() => handleDetailsBadge(badge.badge_id)} className="text-blue-500 hover:text-blue-700 cursor-pointer">
                      <IoEyeSharp />
                    </button>
                    <button onClick={() => handleEditBadge(badge.badge_id)} className="text-gray-500 hover:text-gray-700 cursor-pointer">
                      <FaEdit />
                    </button>
                    <button
                    ...
                    >
                      {loadingBadgeId === badge.badge_id ? "Deleting..." : <RiDeleteBin6Line />}
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <p className="text-center text-gray-500 dark:text-gray-400">No Badges found.</p>
      ) : (
